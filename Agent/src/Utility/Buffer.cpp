...
CBuffer::CBuffer(size_t len /* = DEFAULT_SIZE */,CMemPoolBase * pool /*= NULL*/)
:_flag(0),
_rpos(0),
_wpos(0),
_size(len),
...
{
    assert(size > 0);
    return ((NULL == _pool) ? (uint8 *)PoolMalloc(size) : (uint8 *)_pool->Alloc(size));
}
void    CBuffer::releaseBuf(uint8 * p) 
{
    assert(NULL != p);
    (NULL == _pool) ? PoolFree(p) : _pool->Free(p);
}
