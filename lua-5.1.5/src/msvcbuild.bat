...
@setlocal
@set LUA_COMPILE=cl /nologo /O2 /W3 /c /D_CRT_SECURE_NO_DEPRECATE
...
%LUA_COMPILE% /DLUA_BUILD_AS_DLL lua.c
@if errorlevel 1 goto :BAD
...
%LUA_COMPILE% /DLUA_BUILD_AS_DLL luac.c print.c
%LUA_LINK% /out:luac.exe luac.obj print.obj %LUA_LIBNAME%
if exist luac.exe.manifest^
  %LUA_MT% -manifest luac.exe.manifest -outputresource:luac.exe

@del *.obj *.manifest *.ilk *.pdb
@echo.
@echo === Successfully built LuaJIT for Windows ===
